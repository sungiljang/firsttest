...
$$\displaystyle \frac{-b - \sqrt{b^2 - 4ac}}{2a} {\tt \;and\,} \frac{-b + \sqrt{b^2 - 4ac}}{2a}$$
...
Now since these values are saved in variables, to obtain a solution to our question, we use the quadratic formula:
```{r}
...
data()
```
This shows you the object name for these datasets. Theses datasets are objects that can be used by simply typing the name. For example, if you type:
```{r}
co2
```
...
Values remain in the workspace until you end your session or erase them with the function `rm`. But workspaces also can be saved for later use. In fact, when you quit R, the program asks you if you wnat to save your workspace. If you do save it, the next time you start R, the program will restore the workspace. 
...
(-b - sqrt(b^2 - 4*a*c)) / (2*a)
```
By creating and saving a script with the code above, we would not need to retype everyting each time and, instead, simply change the variable names. Try writing the script above into an editor and notice how easy it is to change the variables and receive an answer.
...
data(murders)
```
To see taht this is in fact a data frame, we type:
```{r}
class(murders)
#> [1] "data.frame"
```
...
```
This tells us much about the object. We see that the table has 51 rows (50 states plus DC) and five variables. We can show the first six lines using the function `head`:
```{r}
head(murders)
...
Thi object `murders$population` is not one number but several. We call these types of objects *vectors*. A single number is technically a vector of length 1, but in general we use the term vectors to refer to objects with several entries. The function `length` tells you how many entries are in the vector:
```{r}
...
Another important type of vectors ar *logical vectors*. These must be either `TRUE` or `FALSE`.
```{r}
z <- 3 == 2
z
#> [1] FALSE
...
data(murders)
```
Use the function `str` to examine the structure of the `murders` object. We can see that this object is a data frame with 51 rows and five columns. Which of the following best describes the variables represented in this data frame? `#C`
