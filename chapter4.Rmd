...
```
Here is a very simple example that tells us which states, if any, have a murder rate lower than 0.5 per 100,000. The `if` statement protects us from the case in which no states satisfies the condition.
```{r}
...
```
This table helps us see what happend:
...
Here is an example of how this function can be readily used to replace all the missing valuse in a vector with zeros:
```{r}
data(na_example)
...
#> [1] 0
```
Two other useful functions are `any` and `all`. The `any` function takes a vector of logicals and returns `TRUE` if any of entries is `TRUE`. The `all` function takes a vector of logicals and returns `TRUE` if all of the entries are `TRUE`. Here is an example:
```{r}
z <- c(TRUE, TRUE, FALSE)
any(z)
#> [1] TRUE
...
```
The functions you define can have multiple arguments as well as default values. For example, we can define a function that computes either the arithmetic or geometric averagey depending on a user defined variable like this:
```{r}
...
Once you start becoming more of an R expert user, you will likely need to load several add-on packages for some of your analysis. Once you start doing this, it is likely that two packages use the same name for two different functions. And often these functions do completely different things. In fact, you have already encountered this because both **dplyr** and the R-base **stats** package define a `filter` funtion. There are five other examples in **dplyr**. We know this because when we first load **dplyr** we see the following message:
```{r}
...
```
How can we compute $S_n$ for various values of $n$, say $n = 1,...,25$? Do we write 25 lines of code calling `compute_s_n`? No, that is what for-loops are for in programming. In this case, we are performing exactly the same task over and over, and the only thing that is changing is the value of $n$. For-loops let us define the range that our variable takes (in our example $n = 1,...,10$), then change the value and evaluate expression as yo *loop*.
...
plot(n, s_n)
```
![](https://rafalab.github.io/dsbook/book_files/figure-html/sum-of-consecutive-squares-1.png)
...
```
2. Which of the following expressions is always `FALSE` when at least one entry of a logical vector `x` is TRUE? `D`
...
3. The function `nchar` tells you how much characters long a character vector is.

Write a line of code that assigns to the object `new_names` the state abbreviation whent the sate name is longer than 8 characters.
```{r}
library(dslabs)
data(murders)
...
```
6. After running the code below, what is the value of `x`? `3`
```{r}
x <- 3
my_func <- function(y){
  x <- 5
...
```{r}
n <- 1:25
S_n_1 <- sapply(n, compute_s_n)
sn2 <- vector("numeric", length = 25)
S_n_2 <- for(i in 1:25){
  sn2[i] <- i*(i+1)*(2*i+1)/6
}
identical(S_n_1, sn2)
