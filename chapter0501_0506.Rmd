...
1. Examine the built-in dataset `co2`. Which of the following is true: `D?`
...
D. `co2` is not tidy: to be tody we would have to wrangle it to have three columns (year, month and value), then each co2 observation would have a row.

2. Examine the built-in dataset `ChickWeight`. Which of the following is true: `B?`
...
3. Examine the built-in dataset `BOD`. Which of the following is true: `C?`
...
4. Which of the following built-in datasets is tidy (you can pick more than one): `C,D,E?`
...
The **dplyr** package from **tidyverse** introduces functions that perform some of the most common operations when working with data frames and uses names for these functions that are relatively easy to remember. For instance, to change the data table by adding a new column, we use `mutate`. To filter the data table to a subset of rows, we use `filter`. Finally, to subset the data by selecting specific columns, we use `selelct`.

### 5.3.1 Adding a column with `mutate`

We want all the necessary information for our analysis to be included in the data table. So the first task is to add the murder rates to our murders data frame. The function `mutate` takes the data frame as a first argument and the name and values of the variables as aa second argument using the convention `name = values`. So, to add murder rates, we use:
...
c
```
Notice that here wa used `total` and `population` inside the function, which are objects that are **not** defined in our workspace. But why don't we get an error?
...
# 그렇다면 mutate의 argument로 다른 dataframe의 변수를 불러오는 건 가능한가?
# 예를 틀어 mutate(murders, rate = nomurder$total / nomurder$population) 이렇게? -> 된다!
```{r}
murderstest <- mutate(murders, co2co2 = co2[1:51])
head(murderstest)
#>        state abb region population total     rate co2co2
#> 1    Alabama  AL  South    4779736   135 2.824424 315.42
#> 2     Alaska  AK   West     710231    19 2.675186 316.31
#> 3    Arizona  AZ   West    6392017   232 3.629527 316.50
#> 4   Arkansas  AR  South    2915918    93 3.189390 317.56
#> 5 California  CA   West   37253956  1257 3.374138 318.13
#> 6   Colorado  CO   West    5029196    65 1.292453 318.00
```
...
# head(select(murders, state, population)) 와 같은 결과
```
...
#>                  state abb        region population total population_in_millions      rate rank
#> 1 District of Columbia  DC         South     601723    99               0.601723 16.452753    1
#> 2            Louisiana  LA         South    4533372   351               4.533372  7.742581    2
#> 3             Maryland  MD         South    5773552   293               5.773552  5.074866    4
#> 4             Missouri  MO North Central    5988927   321               5.988927  5.359892    3
#> 5       South Carolina  SC         South    4625364   207               4.625364  4.475323    5

# rank 순서대로 나오는 것은 아님
filter(murders, 2 <= rank <= 5) # 에러 남
filter(murders, rank >= 2, rank <= 5) # 가능
filter(murders, rank < 11, region == "South") #두 조건 모두 만족하는 것만 (and) 출력됨. or 로 하려면?
filter(murders, rank < 11 | region == "South") # rank < 11 또는 (or) region == "South" 인 것 출력됨
```
```{r}
logical operators
&: 논리곱(and)- 벡터 전체 결과
...
isTRUE()- 벡터 길이가 1이면서 TRUE일 경우 TRUE
```
...
nrow(no_south)
#> [1] 34
#> length 의 경우 variable의 개수 출력 -> names(no_south) 에서 출력되는 8개의 variable
```

6. We can also use %in% to filter with **dplyr**. You can therefore see the data from the New York and Texas like this:
```{r}
filter(murders, state %in% c("New York", "Texas"))
# filter(murders, state == "New York" | state == "Texas") 같은 결과
```
...
$$\mathrm{original\,data\,\rightarrow\,select\,\rightarrow\,filter}$$
...
murders %>% select(state, region, rate) %>% filter(rate <= 0.71)
```
`murders` is the first argument of the `select` funcion, and the new data frame (formerly `new_table`)is the first argument of the `filter` function.
...
```
In the solution to be the previous exercise, we did the following:
```{r}
my_states <- filter(murders, region %in% c("Northeast", "West") & rate < 1)
